...
1. Read the spec and its tickets — `mcp__meldom__ticket_view({ "id": <spec-id> })` for the spec body, then its children with their `blocked_by` edges. Read enough to understand the task graph. Read attached `notes[]` and `attachments[]`; an attachment's local `path` is Readable.
