...
Completion means every in-scope ticket and acceptance criterion is implemented, verified, reviewed, and recorded. A parent includes all its open children as of step 1, unless the user explicitly narrows the scope; work that surfaces after that read is a follow-up (step 3), built by the next run (step 6).
