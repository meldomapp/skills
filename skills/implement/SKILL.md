...
5. **Review, then record the outcome.** When the work is done, call the Skill tool with `meldom:review` over the changes. Its Spec axis is what checks each ticket against its acceptance criteria, so read that report per ticket and set `mcp__meldom__ticket_outcome({ "id": <id>, "outcome": "verified" })` for the ones it confirms. A ticket the report contradicts gets `"outcome": "failed"` and goes back to `open` — do not leave it reading `done`.
