...
The **scope** is the set of tickets handed to you at step 1 — a parent means all its open children as of that read, unless the user explicitly narrows it — plus **every ticket this session creates**, whatever its type, cause or age. A bug you found beside the seam, a gap the spec skipped, a pre-existing fault in a test or a script: if it is worth a ticket, it is in the scope and you build it here. There is no category of ticket you file and leave. Completion means every ticket in the scope reads `done` or `closed`, each implemented, verified and reviewed in this session. The final message says exactly that, per ticket. It never lists an open ticket, a next step, or anything left for the user — there is nothing left, or you are not finished.
...
3. **Implement at the pre-agreed seams.** Call the Skill tool with `meldom:tdd` where a seam is testable — write the failing test at that seam, then the minimal code to pass. Reach for the Skill tool with `meldom:codebase-design` when an interface needs designing.
   ...
   Work an in-scope acceptance criterion cannot pass without is in scope: build it. Everything else the build surfaces is a **follow-up**: file it the moment you see it with `mcp__meldom__ticket_create({ "title": "...", "body": "<what you saw, where, and its acceptance criteria>", "parent_id": "<parent ulid>" })`, return to the criterion you were on, and build it in the next pass (step 6). A follow-up lives inside the family of the ticket you are building, so `parent_id` is always passed: the spec from step 1, or the ticket's own parent. When the ticket you are building is a root with no parent, give it one first — `ticket_create({ "title": "<the ticket's title>", "body": "<one line naming the ticket it groups>", "type": "spec", "parent_id": null })`, then `ticket_update({ "id": "<ticket ulid>", "parent_id": "<new spec ulid>" })` — and file under that spec. A ticket this session filed as a board root is a mistake; reparent it at once.
